...
Algae <- read.csv('algae.csv',header = T,row.names = 1)
Cyanobacteria <- read.csv('cyanobacteria.csv',header = T,row.names = 1)
Rotifer <- read.csv('rotifer.csv',header = T,row.names = 1)
Cladocera <- read.csv('cladocera.csv',header = T,row.names = 1)
Copepod <- read.csv('copepod.csv',header = T,row.names = 1)
...
DN_class <- read.csv('class.csv',header = T,row.names = 1)
dn.table <- DN_class
...
for(group in groupes.names){
  print(group)
  Mxp.plot.loc=Mxp.plot[dn.table[which(dn.table[,2]==group),1],]
...
write.csv(BICgraph.med,'Low_result.csv')
