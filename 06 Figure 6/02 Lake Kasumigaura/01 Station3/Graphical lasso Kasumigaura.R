env <- read.csv("03 env_sta3_final.csv",header = T, row.names = 1)
library(vegan)
library(ggplot2)
library(ggrepel)


Algae <- read.csv('algae_sta3.csv',header = T,row.names = 1)
Cyanobacteria <- read.csv('cyanobacteria_sta3.csv',header = T,row.names = 1)
Rotifer <- read.csv('rotifer_sta3.csv',header = T,row.names = 1)
Cladocera <- read.csv('cladocera_sta3.csv',header = T,row.names = 1)
Copepod <- read.csv('copepod_sta3.csv',header = T,row.names = 1)
...
Station3_class <- read.csv('class_station3.csv',header = T,row.names = 1)
Station3.table <- Station3_class
...
for(group in groupes.names){
  print(group)
  Mxp.plot.loc=Mxp.plot[Station3.table[which(Station3.table[,2]==group),1],]
...
write.csv(BICgraph.med,'Station3_result.csv')
