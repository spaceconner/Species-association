env <- read.csv("03 env_sta9_final.csv",header = T, row.names = 1)
library(vegan)
library(ggplot2)
library(ggrepel)


Algae <- read.csv('algae_sta9.csv',header = T,row.names = 1)
Cyanobacteria <- read.csv('cyanobacteria_sta9.csv',header = T,row.names = 1)
Rotifer <- read.csv('rotifer_sta9.csv',header = T,row.names = 1)
Cladocera <- read.csv('cladocera_sta9.csv',header = T,row.names = 1)
Copepod <- read.csv('copepod_sta9.csv',header = T,row.names = 1)
...
Station9_class <- read.csv('class_station9.csv',header = T,row.names = 1)
Station9.table <- Station9_class
...
for(group in groupes.names){
  print(group)
  Mxp.plot.loc=Mxp.plot[Station9.table[which(Station9.table[,2]==group),1],]
...
write.csv(BICgraph.med,'Station9_result.csv')
